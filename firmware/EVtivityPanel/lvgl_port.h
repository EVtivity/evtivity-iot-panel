 ...
 *
 * Mode 3 (double-buffer direct-mode): the configuration Waveshare ships and
 * validates on the 7B LVGL demos. The flicker on content updates was not the
 * render mode but the UI doing a full lv_obj_clean + rebuild on every live value
 * change; that churn is now avoided by updating values in place (see ui.cpp
 * ui_set_session / ui_set_connectors). Pairs with EXAMPLE_LCD_RGB_BUFFER_NUMS = 2
 * and the bounce buffer (sync via on_bounce_frame_finish).
 */
